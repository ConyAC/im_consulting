...
import cl.koritsu.im.view.LoginView;
...
public final class ImUI extends UI implements ErrorHandler {

	private transient static Logger logger = LoggerFactory.getLogger(ImUI.class);
	...
	public void updateContent() {
		...
		if (user != null) {
			...
			if (SecurityHelper.isLogged()) {
				...
				// si no est� logeado y la siguiente vista no es el login, lo redirige
			}
	...
	// VaadinSession.getCurrent().getAttribute(Constants.SESSION_USUARIO);
	...
	// !getNavigator().getState().equals("en_proceso"))
	...
	public static ErrorMessage getErrorMessageForException(Throwable t) {
		...
		if (validationException != null) {
			for (ConstraintViolation<?> c : validationException.getConstraintViolations())
				logger.error("Error al validar {}.{} = {} \n{}", c.getRootBeanClass(), c.getPropertyPath(),
						c.getInvalidValue(), c.getMessage());
			return new UserError(validationException.getMessage(), AbstractErrorMessage.ContentMode.TEXT,
		...
		if (persistenceException != null) {
			...
			return new UserError(persistenceException.getLocalizedMessage(), AbstractErrorMessage.ContentMode.TEXT,
					ErrorMessage.ErrorLevel.ERROR);
		}
		...
		if (commitException != null) {
			...
			return new UserError(commitException.getMessage(), AbstractErrorMessage.ContentMode.TEXT,
					ErrorMessage.ErrorLevel.ERROR);
		}
		logger.error("Error al comitear.", t);
		return new UserError(
				"Error interno, tome una captura de pantalla con el error y cont�ctese con el administrador del sistema.",
				AbstractErrorMessage.ContentMode.TEXT, ErrorMessage.ErrorLevel.ERROR);
	}
